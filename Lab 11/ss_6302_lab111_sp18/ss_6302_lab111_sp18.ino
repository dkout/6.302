...
float angleOffsetV = -0.14;  // RESET THIS VALUE (-0.2->0.2) so precisely horizontal arm --> precisely zero angle
float directV=2.9;  // RESET THIS VALUE with zero state gains so the prop is nominally horizontal
float RmotorOverRsense = 0.75;  // RESET THIS value (0.5->1.5) to get bemf=zero when motor is stopped
float nominalBemfV = 1.19; // RESET THIS VALUE of motorBemf when horizontal, to use in  linearization

float Kangle = 29.2373; // Gain for arm angle state
float Komega = 4.0251; // Gain for arm rotational velocity state
float Kemf=11.6741;  // Gain for backEmf1 state
float Kint = 97.0569; //Gain for integral of error 
float Kr = 0; // Change Kr with K!!
