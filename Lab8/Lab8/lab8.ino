...
float directV = 2.79;  // RESET THIS VALUE with zero state gains so the prop is nominally horizontal
float nominalBemfV = 0.0; // RESET THIS VALUE of motorBemf when horizontal, to use in  linearization
float angleOffsetV = 0.0;  // RESET THIS VALUE precisely horizontal arm --> precisely zero angle

// Gains
float Kangle = 0.0; // Gain for arm angle state
float Komega = 0.0; // Gain for arm rotational velocity state
float Kemf= 0.0;  // Gain for backEmf state
float Kr = 0.0; // Change Kr with K!! 
