...
#define alternatePeriod 0.1 // Time Between alts in seconds
...
#define vReadADC 5.0 // Assumes all signals are 0->5 volts and then multiplied by 2/3.
#define vDriveDAC 3.3 // DAC maximum is 3.3
#define vDrivePWM 5.0 // Assumes a 5v driven H-bridge
...
void loop() {  // Main code, runs repeatedly
  ...
  float FieldV = vReadADC * float(FieldI) / float(analogAverages*adcMax);
  ...
  float motorCmdLim = min(max(motorCmd, 0), vDrivePWM);
  analogWrite(motorOutPWM, int((motorCmdLim / vDrivePWM)*dacMax));
  motorCmdLim = min(max(motorCmd, 0), vDriveDAC);
  analogWrite(A14,int((motorCmdLim/vDriveDAC)*dacMax));

  // Print Measured Field
  Serial.println(1000.0*FieldV);
}
