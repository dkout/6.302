...
// Calibration Variables (First angleOffset, then directV, then RmotorOverRsense, then nominalBemfV)
float angleOffsetV = -0.02;  // RESET THIS VALUE (-0.2->0.2) so precisely horizontal arm --> precisely zero angle
float directV = 2.9;  // RESET THIS VALUE (1.5->4.0) with so the prop is nominally horizontal
float RmotorOverRsense = 0.8;  // RESET THIS value (0.5->1.5) to get bemf=zero when motor is stopped
float nominalBemfV = 1.12; // RESET THIS VALUE to the motorBemf when horizontal, to use in  linearization

// Gains (When Calibrating, use PD controller with Kangle = Kr = 2.0, other K's = 0.)
float Kangle = 20.289; // Gain for arm angle state
float Komega = 2.053; // Gain for arm rotational velocity state
float Kemf= 7.9627;  // Gain for backEmf state.
float Kint = 100;  // Gain for the integral state.
float Kr = 20.289; // Gain for the reference input.
...
String config_message = "&A~Desired~5&C&S~Desired~A~-2.0~2.0~0.05&T~Theta~F4~-1.0~1.0&T~Omega~F4~-10.0~10.0&T~Vbemf~F4~-1.0~5.0&T~Error~F4~-3.0~3.0&T~MCmd~F4~0~5&D~100&H~4&";
...
void loop() {  // Main code, runs repeatedly 
  ...
  float motorCmd = directV + Kr*desiredAngleV - (Kemf*deltaVemf + Komega*omegaV + Kangle*angleV + Kint*errorVintegral);
  ...
  if (loopCounter == numSkip) {  
    packStatus(buf, angleV, omegaV, deltaVemf, 0.1*errorVintegral, motorCmdLim, float(headroom));
    Serial.write(buf,26);
    loopCounter = 0;
    headroom = deltaT;
  } else loopCounter += 1;
