...
//Rest of Setup:
bool first_time;
String config_message_30_bytes = "&A~DesiredAng~5&C&S~d~O~0~5.0~0.1&S~DesiredAng~A~-1~1~0.1&S~Kp~P~0~20~0.1&S~Kd~D~0~5~0.1&T~Angle~F4~-2.5~2.5&T~Error~F4~-5~5&T~Deriv~F4~-10~10&T~Cmd~F4~0~5&D~100&H~4&";
...
void loop() {  // Main code, runs repeatedly
  ...
  float errorVdelta = (errorV-pastErrorV[past_size-1])*scaleD;
  ...
  if (loopCounter == numSkip) {  // Lines below are for debugging.
    packStatus(buf, angleV, errorV, errorVdelta, motorCmdLim, float(headroom));
    Serial.write(buf,22);
    loopCounter = 0;
    headroom = deltaT;
  } else loopCounter += 1;
